function [b,fval,exitflag,output] = fzero02601(FunFcnIn,x,options,varargin)
...
if ~isa(x,'double')
  error(message('MATLAB:fzero:NonDoubleInput'))
end
...
elseif (numel(x) == 1)
    if trace > 2 
        disp(' ')
        fprintf(getString(message('MATLAB:fzero:SearchForIntervalContainingSignChange',sprintf('%g',x))));
    ...
    if trace > 2
        ...
        fprintf('%5.0f   %20.16g %13.6g %13.6g %13.6g   %s\n',fcount,a,fa,b,fb, procedure);
    end
    ...
    if trace > 2
        disp(' ')
        fprintf(getString(message('MATLAB:fzero:SearchForZeroInInterval',sprintf('%g',a),sprintf('%g',b))));
    end
...
if abs(fval) <= max(abs(savefa),abs(savefb))
    msg = '  Zero found';%sprintf(getString(message('MATLAB:fzero:ZeroFoundInInterval',sprintf('%g',savea),sprintf('%g',saveb))));
else
    exitflag = -5; 
    msg = sprintf(...
