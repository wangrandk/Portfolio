...
% the size of x is alway a multiplication of sigma.
% of rounded sigma.
Ix= filter2(gx,im,'same'); %  row gx(+/0/-)
Ix= filter2(g',Ix,'same'); %  column g' (+)
Iy= filter2(g,im,'same');  % row g(+)
Iy= filter2(gx',Iy,'same'); %  column gx(+/0/-)
Ixy = filter2(gx, Ix.*Iy, 'same');
Ixy1 = filter2(g, Ix.*Iy, 'same');
figure(1)
subplot(1,2,1)
imshow(Iy);
subplot(1,2,2)
