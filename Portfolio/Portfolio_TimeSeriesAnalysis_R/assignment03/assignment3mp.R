...
diagtool <- function(residuals){
  ...
  acfpl <- acf(residuals, lag.max = 40,plot=FALSE)
  ...
  pacfpl <- pacf(residuals, lag.max =40,plot=FALSE)
  ## Transform the lags from years to months
...
adfTest(Dx1)
library(forecast)
auto.arima(x1, d=NA, D=NA, max.p=5, max.q=5,
...
(m1 <- arima(x = x1, order=c(1,0,1),seasonal = list(order=c(0,1,1), period=12) ))
...
p <- predict(m1,h=8,level=95)
p
str(p)
plot (x1,xlim=c(1995, 2003), type="o")
points(p$pred,col=2,type="o")
...
izModela <- c(fitted.Arima(m1), pred$mean) 
