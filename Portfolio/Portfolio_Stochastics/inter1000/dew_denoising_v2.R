...
#image(seq(0,1,l=nx),seq(0,1,l=ny),dew,
#      main="Original Image",
#      xlab="",ylab="",
#      col=c("white","black"))
#image(seq(0,1,l=nx),seq(0,1,l=ny),noisy.dew,
...
png("dew_MassWrngPix-iter100_beta2.png",height=300)
par(mfrow=c(1,2)) 
plot(seq(1,niter,1),c(0,rep(1,niter-1)),type='n',
     main='mass of blacks (ones)', xlab='iter',ylab='mass')
for(b in 1:nb){lines(seq(1,niter,1),mass[,1],type='l',col=b,lwd=1.5)}
# legend
# lines(seq(1,niter,1),mass[,1],type='l',lwd=1.5)
txt2=c("beta=0.5","beta=1.0","beta=1.5","beta=2.0")
legend("bottomright",txt2, col=c(1,2,3,4),lty=c(1,1,1,1),cex=0.78)

plot(seq(1,niter,1),c(0,rep(0.2,niter-1)),type='n',
     main='wrong pixels', xlab='iter',ylab='#/ntot')
for(b in 1:nb){lines(seq(1,niter,1),WrongPixels[,1]/ntot,type='l',col=b,lwd=1.5)}
# lines(seq(1,niter,1),WrongPixels[,1]/ntot,type='l',lwd=1.5)
dev.off()