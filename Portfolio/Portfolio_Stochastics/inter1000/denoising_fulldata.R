...
beta=2
niter=40
burn.in=20 # time when PixDiff stabilizes
...
ChangingPix <- matrix(nx,ny,data=0)
...
for(t in 2:niter){
  ...
  for(i in 1:nx){
    for(j in 1:ny){
      ...
      if(t>=burn.in && accept==1){ChangingPix[i,j]=ChangingPix[i,j]+1}
    } # 1:changed, 0:unchanged
    
  }
  #Count the different pixels with respect to previous elements
  #We can see the convergence here, at the end, no more pixels will be converted based on nabo
  PixDiff[t]=sum(x!=y)
...
proc.time() - ptm
max_ChPix=max(as.vector(ChangingPix))


png("dataFig_beta1_.png")
...
lines(seq(1,niter,1),PixDiff/ntot,type='l',col=1,lwd=1.5)
...
lines(seq(1,niter,1),mass[1:40],type='l',col=1,lwd=1.5)
dev.off()
...
png("mass_pixdiff_.png",height= 250, width=550)
par(mfrow=c(1,2),pty = "s",mex=.7,pin=c(2,2),cex=0.9) 
...
plot(seq(1,niter,1),c(0,rep(0.25,niter-1)),type='n',
     main='Pixdiff with previous iter', xlab='iter',ylab='#/ntot')
lines(seq(2,40,1),beta1[2:40]/ntot,type='l',col=1,lwd=1)
lines(seq(2,40,1),beta1.5[2:40]/ntot,type='l',col=2,lwd=1)
lines(seq(2,40,1),beta2[2:40]/ntot,type='l',col=3,lwd=1)
txt1=c("beta=1.0","beta=1.5","beta=2.0")
legend("topright",txt1, col=c(1,2,3),lty=c(1,1,1))

plot(seq(1,niter,1),c(0,rep(1,niter-1)),type='n',
     main='mass of blacks', xlab='iter',ylab='mass')
lines(seq(1,niter,1),mass1,type='l',lwd=1)
lines(seq(1,niter,1),mass1.5,type='l',lwd=1)
lines(seq(1,niter,1),mass2,type='l',lwd=1)
legend("topright",txt1, col=c(1,2,3),lty=c(1,1,1))
# image(seq(0,1,l=nx),seq(0,1,l=ny),ChangingPix/max_ChPix,
#       main=paste("ChangingPix after burnin ",burn.in),
#       xlab="",ylab="",
...
for(i in 1:50) {
  for(j in 1:50) {
    patch[i,j]=x[i,j]
    smallmask[i,j] <- ifelse(ChangingPix[i,j]==0,0,1)
  }
}
...
image(seq(0,1,l=nx),seq(0,1,l=ny),x,
      main="Reconstruction(500x500)",
      xlab="",ylab="",
      col=c("black","white"))
#draw.circle(.1,.1,.08,nv=10,border=2,col=NA,lty=1,lwd=3)
rect(0,0,.1,.1,density = NULL,col = NA, border =2,lty=1,lwd=2)
