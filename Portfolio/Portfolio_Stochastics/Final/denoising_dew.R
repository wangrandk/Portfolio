...
library(png)
file = readPNG("d.png")
pr=pixmapGrey(file,nrow=256/4,ncol=256/4,bbox=NULL, bbcent=FALSE, cellres=2)
nx=256/4;ny=256/4; ntot = nx* ny
chess=matrix(nr=nx,nc=ny,data=pr@grey)
chess=ifelse(chess>.9,1,0)
chess = t(chess)
noisy.chess=chess+rnorm(ntot,0,0.3)

image(seq(0,nx,l=nx),seq(0,ny,l=ny),chess,
...
for(t in 2:niter){
  for(i in 1:nx){
    for(j in 1:ny){
      ...
      R=(prior(clone,i,j,beta) * dnorm(x,xcand,sigma))/(prior(x,i,j,beta)* dnorm(x,x[i,j],sigma))
      #R=prior(clone,i,j,beta)/prior(x,i,j,beta)
      #R=R*dnorm(xcand,x[i,j],var)/dnorm(x[i,j],xcand,var)
      #the probability p
      p=min(1,R)
...
png("denoising_dew.png",height=300)
par(mfrow=c(1,2))
image(seq(0,1,l=nx),seq(0,1,l=ny),x0,
      main="ChessBoard at time 0",
      xlab="x",ylab="y",
