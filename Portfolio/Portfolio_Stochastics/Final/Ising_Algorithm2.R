...
for(i in 1:nx){
  for(j in 1:ny){
    noisy.dew[i,j] <- ifelse(noisy.dew[i,j]>0.5,1,0)  
  }
}
xini=x <- noisy.dew
...
png("Ising2_dew_iter300_beta2.png",height= 1000, width=750)
...
for(t in 2:niter){
  ...
  #Evolutionplot
  if(t %% 30 ==0){ #time to see the changes
...
png("dew_HM_iter300.png",height=300)
par(mfrow=c(1,2))
image(seq(0,1,l=nx),seq(0,1,l=ny),xini,
      main="Chain at time 0",
      xlab="x",ylab="y",
