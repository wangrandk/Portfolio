...
ntot = nx*ny
library(png)
file = readPNG("chess-board.png")

#file= 'http://www2.imm.dtu.dk/courses/02443/projects/data/potts.txt'
x0 <- y <-  matrix(nr=292, nc= 268,read.table(file))
...
png("binary_patch.png",height=300)
par(mfrow=c(1,2))
image(seq(0,1,l=nx),seq(0,1,l=ny),smallx,
      main="Chain at time 0",
      xlab="x",ylab="y",
...
s <- matrix(nr=50, nc= 50,data=0)
for (i in 1:nr){
  for (j in 1:nc){
    s[pi*(1+5)^2,pi*(1+5)^2] = 1
  }
}
nr=500; nc= 500; n = nr*nc;
x=matrix(nr,nc,data=rbinom(n=n,size=1,prob=0.01));

image(seq(0,537,l=537),seq(0,713,l=713),image,
      main="Chain at time 0",
      xlab="x",ylab="y",
      col=c(0,1))

library(plotrix)
draw.circle(50,300,50,col=1)
draw.circle(300,30,25,col=1)
draw.circle(200,200,65,col=1)
draw.circle(400,400,45,col=1)
draw.circle(350,200,55,col=1)
library(png)
img <- readPNG("Rplot.png",native = FALSE, info = FALSE)
m <- as.raster(img,max=255)
dev2bitmap("Rplot.png", type = "png16m", height = 537, width = 713)
save(img,file="img.Rdata")
image <- as.matrix(img,nrow=537,ncol=713,byrow = F)
image= t(image)
image <- matrix(img,nrow=537,ncol=713,byrow = T)

library(pixmap)
library(bmp)
r=read.bmp("Rplot.bmp",Verbose = FALSE)
pr=pixmapRGB(r)
